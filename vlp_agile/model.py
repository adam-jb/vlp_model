...
def build_pl(vpp_rev_base, scaling_factors):
    """
    Build VLP company P&L.
    Consumer pays for battery — no depreciation, no loan, no interest.

    Revenue = VLP income only (wholesale arb + FR + CM + BM)
    Opex = staff + customer management (VLP)
    No COGS, no network charges, no levy, no depreciation, no financing costs.
    """
...
def build_cashflow(vpp_rev_base, scaling_factors):
    """Build VLP company cashflow.
    Consumer pays for battery — no loan repayments, no interest.
    Revenue = VLP income only. Only cash outflow: opex + CAC (yr 1)."""
    opex = STAFF_COST + CUST_MGMT
    cum_cash = 0.0
    rows = []
    for yr in range(1, MODEL_YEARS + 1):
...
def plot_consumption_comparison(all_results, spread_name):
    """Compare consumption levels for a spread scenario."""
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(11, 7), sharex=True)
    fig.suptitle(
        f"VLP Agile Model  |  {spread_name} spread\n"
        f"Company Free Cashflow by Consumption Level (no battery cost)",
        fontsize=11, fontweight="bold", y=0.99)

    years = np.arange(1, MODEL_YEARS + 1)
    for ckwh in CONSUMPTION_LEVELS:
        key = (spread_name, ckwh)
...
def plot_spread_comparison(all_results, consumption):
    """Compare spread scenarios for a consumption level."""
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(11, 7), sharex=True)
    fig.suptitle(
        f"VLP Agile Model  |  {consumption:,} kWh/yr\n"
        f"Spread Scenarios Compared (no battery cost)",
        fontsize=11, fontweight="bold", y=0.99)

    years = np.arange(1, MODEL_YEARS + 1)
    for sp_name in SPREAD_SCENARIOS:
        key = (sp_name, consumption)
        cf = all_results["cf"][key]
...
def _build_report_js(milp_results, all_scaling):
    ...
    data = {
        "BAT_COST": BAT_COST, "CORP_TAX": CORP_TAX,
        "MODEL_YEARS": MODEL_YEARS, "DEGRADATION_PA": DEGRADATION_PA,
        "CONSUMPTION_LEVELS": CONSUMPTION_LEVELS,
...
function buildCF(vppRev,sf,inp){
    ...
    for(var yr=1;yr<=D.MODEL_YEARS;yr++){
        ...
        var rev=vppRev*s*df;
        var ebitda=rev-opex;
        var ebt=ebitda, tax=Math.max(0,ebt*D.CORP_TAX), ocf=ebitda-tax;
        var cacOut=yr===1?inp.cac:0, fc=ocf-cacOut; cum+=fc;
        rows.push({yr:yr,rev:rev,s:s,df:df,opex:opex,
            ebitda:ebitda,ebt:ebt,tax:tax,netInc:ebt-tax,ocf:ocf,cacOut:cacOut,fc:fc,cum:cum});
    }
    return rows;
}
function renderSummary(){
    ...
    ['Spread','Consumption','VPP Rev £','Opex £','EBITDA £',
     'Yr1 FCF £',D.MODEL_YEARS+'yr Cum £'].forEach(function(c){h+='<th>'+c+'</th>';});
    h+='</tr></thead><tbody>';
    D.spread_names.forEach(function(sp){
        ...
        D.CONSUMPTION_LEVELS.forEach(function(ckwh){
            ...
            var opex=inp.staff+inp.mgmt;
            var cf=buildCF(m.total_vpp,sf,inp);
            var yr1=cf[0].fc, cumF=cf[cf.length-1].cum;
            h+='<tr><td>'+sp+'</td>';
            h+='<td>'+ckwh.toLocaleString()+'</td>';
            h+='<td>'+fmt0(m.total_vpp)+'</td>';
            h+='<td>'+fmt0(opex)+'</td>';
            h+='<td>'+fmt0(m.total_vpp-opex)+'</td>';
            h+='<td>'+fmt0(yr1)+'</td><td>'+fmt0(cumF)+'</td></tr>';
        });
    });
    h+='</tbody></table>';
    document.getElementById('summary-dashboard').innerHTML=h;
}
function renderPLTable(vppRev,sf,inp){
    var opex=inp.staff+inp.mgmt;
    var h='<table class="data-table"><thead><tr>';
    ['Year','VLP Revenue (£)','Spread','Degrade',
     'Staff (£)','Mgmt (£)','Opex (£)','EBITDA (£)','Tax 25% (£)','Net Inc (£)'].forEach(function(c){h+='<th>'+c+'</th>';});
    h+='</tr></thead><tbody>';
    for(var yr=1;yr<=D.MODEL_YEARS;yr++){
        var s=sf[String(yr)]||1, df=degradeFactor(yr);
        var rev=vppRev*s*df;
        var ebitda=rev-opex, ebt=ebitda;
        var tax=Math.max(0,ebt*D.CORP_TAX), ni=ebt-tax;
        h+='<tr><td>'+yr+'</td><td>'+fmt(rev)+'</td>';
        h+='<td>'+s.toFixed(3)+'</td><td>'+df.toFixed(3)+'</td>';
        h+='<td>'+fmt(inp.staff)+'</td><td>'+fmt(inp.mgmt)+'</td><td>'+fmt(opex)+'</td>';
        h+='<td>'+fmt(ebitda)+'</td><td>'+fmt(tax)+'</td>';
        h+='<td>'+fmt(ni)+'</td></tr>';
    }
    h+='</tbody></table>'; return h;
}
function renderCFTable(vppRev,sf,inp){
    var cf=buildCF(vppRev,sf,inp);
    var h='<table class="data-table"><thead><tr>';
    ['Year','VLP Revenue (£)','EBITDA (£)','Tax (£)','Op CF (£)',
     'CAC (£)','Free Cash (£)','Cumulative (£)'].forEach(function(c){h+='<th>'+c+'</th>';});
    h+='</tr></thead><tbody>';
    cf.forEach(function(r){
        var cls=r.fc<0?' class="negative"':' class="positive"';
        h+='<tr><td>'+r.yr+'</td><td>'+fmt(r.rev)+'</td>';
        h+='<td>'+fmt(r.ebitda)+'</td><td>'+fmt(r.tax)+'</td>';
        h+='<td>'+fmt(r.ocf)+'</td>';
...
function renderDetails(){
    ...
    D.spread_names.forEach(function(sp){
        ...
        D.CONSUMPTION_LEVELS.forEach(function(ckwh){
            var m=D.milp[String(ckwh)];
            h+='<details><summary>P&L — '+sp+' spread, '+ckwh.toLocaleString()+' kWh/yr</summary>';
            h+=renderPLTable(m.total_vpp,sf,inp)+'</details>';
            h+='<details><summary>Cashflow — '+sp+' spread, '+ckwh.toLocaleString()+' kWh/yr</summary>';
            h+=renderCFTable(m.total_vpp,sf,inp)+'</details>';
        });
...
def build_html_report(all_results, milp_results, savings_projections, charts, out_path):
    ...
    for sp_name in SPREAD_SCENARIOS:
        detail_sections += f"""
        <div class="section">
        <h2>{sp_name} Spread — VLP Company P&L and Cashflow (consumer pays for battery)</h2>
        """
        # Charts
...
<tbody>
<tr><td>Battery financing</td><td>Consumer pays upfront &mdash; no company loan or depreciation</td></tr>
...
<div class="section">
<h2>VLP Company Economics Dashboard (VLP revenue only, no battery cost)</h2>
...
<div class="assumption">
<strong>10. Consumer pays for battery.</strong>
The P&amp;L and cashflow assume the consumer funds the battery upfront. The company has no
depreciation, no loan repayments, and no interest expense. Only ongoing costs are staff
and VLP customer management.
</div>
...
def main():
    ...
    # No loan dimension — consumer pays for battery, company has no financing costs
    all_results = {"pl": {}, "cf": {}}
    ...
    for sp_name in SPREAD_SCENARIOS:
        ...
        for ckwh in CONSUMPTION_LEVELS:
            ...
            key = (sp_name, ckwh)
            all_results["pl"][key]  = build_pl(vpp_rev, sf)
            all_results["cf"][key]  = build_cashflow(vpp_rev, sf)
    ...
    for sp_name in SPREAD_SCENARIOS:
        for ckwh in CONSUMPTION_LEVELS:
            key = (sp_name, ckwh)
            all_results["pl"][key].to_csv(
                OUT_DIR / f"pl_{sp_name}_{ckwh}.csv", index=False)
            all_results["cf"][key].to_csv(
